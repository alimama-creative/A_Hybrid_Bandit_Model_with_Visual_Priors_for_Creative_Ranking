...
class ContextualDataset(object):
  ...
  def add(self, context, action, reward, tag):
  ...
  def replace_data(self, contexts=None, actions=None, rewards=None,tags=None):
  ...
  def get_specific_data(self, action, tag):
    """Returns all (context, reward) where the action of tag was played ."""
    n, _ = self.contexts.shape
    ind = np.array([i for i in range(n) if self.actions[i] == action and self.tags[i] == tag])
    if ind.shape[0] == 0:
      return None, None
    return self.contexts[ind, :], self.rewards[ind, action]

  def get_shared_data(self, action):
